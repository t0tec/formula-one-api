package org.formulaone.rest.wrapper;

import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonRootName;
import com.fasterxml.jackson.dataformat.xml.annotation.JacksonXmlElementWrapper;
import com.fasterxml.jackson.dataformat.xml.annotation.JacksonXmlProperty;

import org.formulaone.service.dto.SeasonDto;
import org.springframework.data.domain.Page;

import java.util.List;

/**
 * @author t0tec (t0tec.olmec@gmail.com)
 * @version $Id$
 * @since 1.0
 */
@JsonRootName("SeasonTable")
public class SeasonPage extends PageResource<SeasonDto> {

  public SeasonPage(Page<SeasonDto> page, String pageParam, String sizeParam) {
    super(page, pageParam, sizeParam);
  }

  @JacksonXmlElementWrapper(localName = "seasons", useWrapping = false)
  @JacksonXmlProperty(localName = "season")
  @JsonProperty("seasons")
  @Override
  public List<SeasonDto> getContent() {
    return page.getContent();
  }
}
